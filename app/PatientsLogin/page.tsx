...
function PatientsLogin() {
  ...
  const PatientsLoginSubmitHandler = async (
    ...
    try {
      const res = await fetch(`/api/auth/PatientsLogin`, {
        method: "POST",
  ...
  return (
    <form
      onSubmit={PatientsLoginSubmitHandler}
    ...
    >
      <div className="w-[700px] rounded flex justify-center p-20 flex-col bg-white">
        ...
        <p>
          <Link className="text-red-600" href={`/PatientsRegister`}>
            حساب کاربری ندارید؟
          </Link>
        </p>
      </div>
